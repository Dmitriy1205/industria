...
class JobApplicationBloc extends Bloc<JobApplicationEvent, JobApplicationState> {
  ...
  JobApplicationBloc({required this.jobApplicationRepository}) : super(const JobApplicationState.initial()) {
    on<JobApplicationEvent>(_mapEventToState, transformer: droppable());
  }

  Future<void> _mapEventToState(JobApplicationEvent event, Emitter<JobApplicationState> emit) => event.map(applyJob: (e) => _applyJob(e,emit));

  Future<void> _applyJob(_ApplyJobEvent event, Emitter<JobApplicationState> emit) async{
    emit(JobApplicationState.loading());
    try{
      await jobApplicationRepository.applyForJob(jobApplication: event.jobApplicationRequest);
